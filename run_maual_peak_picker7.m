...
function select_ied_file(fig)
    ...
    if isfield(fig.UserData, 'eeg_bin_path')
        ...
        % Show GUI error if subject mismatch
        if ~strcmp(bin_subject, txt_subject)
            err_msg = sprintf('Mismatch between EEG .bin and IED .txt files:\n\nEEG file subject: %s\nIED file subject: %s\n\nPlease ensure they match.', ...
                bin_subject, txt_subject);
            uialert(fig, err_msg, 'Subject Mismatch ❌', 'Icon', 'error');
            return;
        end
    end
...
function update_plot(fig)
    ...
    % === Get time window from dropdown ===
    dd = findobj(fig, 'Tag', 'TimeWindowMenu');
    val = dd.Value;

    switch val
        case '±0.5s', win_sec = 0.5;
        case '±1s',    win_sec = 1.0;
        case '±2s',    win_sec = 2.0;
        case '±5s',    win_sec = 5.0;
        otherwise,     win_sec = 0.5;  % fallback
    end

    pre_samples = round(win_sec * fs);
    post_samples = round(win_sec * fs);
    t = (-pre_samples : post_samples) / fs;  % correct time axis
    ...
    for plotIdx = 1:length(visibleIdx)
        ...
        fprintf('[DEBUG] Extracting EEG trace for %s: [%d:%d] → length = %d\n', ...
        label, start_idx, end_idx, end_idx - start_idx + 1);
...
function changeTimeWindow(fig)
    % Trigger a full redraw using the updated window selection
    update_plot(fig);
end
