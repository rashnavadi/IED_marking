...
function plot_Avg_allIEDs_OfMainCh(fig)
    ...
    fs = fig.UserData.fs;
    t = linspace(-0.5, 0.5, numel(avg_waveform));
...
%     t = linspace(-0.5, 0.5, nTime);  % assuming ±0.5s window
    if isfield(fig.UserData, 'avg_t')
        ...
        t = linspace(-0.5, 0.5, nTime);  % fallback
    end
...
function save_final_avg_template(fig)
    ...
    plot(t, template', 'Color', [0.6 0.6 0.6], 'LineWidth', 1.5); % All channels in gray
    ...
    if ~isempty(main_idx)
        hold on;
        plot(t, template(main_idx, :), 'r', 'LineWidth', 2.5);  % Highlighted in red
