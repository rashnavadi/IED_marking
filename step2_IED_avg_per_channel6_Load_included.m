...
%     % --- Load previously saved averages button
%     btnLoadAvg = uibutton(leftLayout, 'Text', 'Load Saved Averages', ...
%         'ButtonPushedFcn', @(btn,event) load_saved_avg_callback(fig));
%     btnLoadAvg.Layout.Row = 14;
%     btnLoadAvg.Layout.Column = 1;

    % --- Load previously saved onset/peak file button
    btnLoadOnset = uibutton(leftLayout, 'Text', 'Load Onset/Peak File', ...
        'ButtonPushedFcn', @(btn,event) load_saved_onset_peak_times(fig));
    btnLoadOnset.Layout.Row = 15;
    btnLoadOnset.Layout.Column = 1;
...
function select_eeg_file(fig)
    ...
    fig.UserData.currentPage = 1;
    fig.UserData.totalChannels = numel(fig.UserData.channelnames_bipolar);
...
function plot_avg_template(fig, template_key)
    ...
    % Reset axes without destroying persistent overlays
    for ax = axesHandles
        % Only clear EEG traces (line objects), not marker lines
        delete(findall(ax, 'Type', 'line', '-not', 'Tag', 'onsetLine', '-not', 'Tag', 'peakLine'));
        delete(findall(ax, 'Type', 'ConstantLine'));  % optional, removes old EEG baselines
        hold(ax, 'on');
        ax.Title.String = '';
        ax.Color = 'white';
        ax.XColor = 'black';
    ...
    % Update label and finalize plot
    update_subject_info_label(fig);
    drawnow limitrate nocallbacks;

    % === Re-draw onset/peak markers if previously loaded ===
    if isfield(fig.UserData, 'loadedOnsetPeakTable') && ~isempty(fig.UserData.loadedOnsetPeakTable)
        plot_loaded_onset_peaks(fig);
        drawnow limitrate nocallbacks;  % force refresh after overlay
    end
    ...
    if fig.UserData.currentPage < maxPage
        ...
        if isfield(fig.UserData, 'currentTemplateKey') && ...
           isfield(fig.UserData, fig.UserData.currentTemplateKey)
            plot_avg_template(fig, fig.UserData.currentTemplateKey);  % ✅ replot average traces

            % 🟢 Add this block: overlay onset/peak markers if loaded
            if isfield(fig.UserData, 'loadedOnsetPeakTable') && ~isempty(fig.UserData.loadedOnsetPeakTable)
                plot_loaded_onset_peaks(fig);
            end
        else
...
%%
function load_saved_onset_peak_times(fig)
    % Determine base directory from current subject/run
    [ied_folder, ~, ~] = fileparts(fig.UserData.ied_txt_path);
    [parent_dir, ~, ~] = fileparts(ied_folder);
    load_dir = fullfile(parent_dir, 'avg_onset_peak_times');

    if ~exist(load_dir, 'dir')
        uialert(fig, 'No saved onset/peak data folder found.', 'Load Failed');
        return;
    end

    % Let user select the Excel or MAT file
    [file, path] = uigetfile( ...
        { '*.xlsx', 'Excel Files (*.xlsx)'; ...
          '*.mat', 'MAT Files (*.mat)' }, ...
        'Select a saved onset/peak times file', load_dir);
    ...
    fprintf('\n[DEBUG] Loading onset/peak file: %s\n', fullpath);

    switch lower(ext)
        case '.xlsx'
            % === Load Excel File ===
            tbl = readtable(fullpath);
            expectedCols = {'Channel', 'PeakAmplitude', 'AvgOnsetTime', 'AvgPeakTime'};
            missingCols = setdiff(expectedCols, tbl.Properties.VariableNames);
            if ~isempty(missingCols)
                msg = sprintf('Excel file missing expected columns:\n%s', strjoin(missingCols, ', '));
                uialert(fig, msg, 'Format Error');
                return;
            end

            tbl = tbl(~all(ismissing(tbl),2), :);

            abs_onsets = tbl.AvgOnsetTime;
            abs_peaks  = tbl.AvgPeakTime;
            ch_labels  = string(tbl.Channel);

        case '.mat'
            % === Load MAT File ===
            s = load(fullpath);
            if isfield(s, 'onset_times') && isfield(s, 'peak_times')
                abs_onsets = s.onset_times;
                abs_peaks  = s.peak_times;
                if isfield(s, 'channelnames')
                    ch_labels = s.channelnames;
                else
                    ch_labels = strings(size(abs_onsets));
                end
            else
                uialert(fig, 'MAT file missing required variables (onset_times, peak_times).', 'Format Error');
                return;
            end
    end

    % === Convert to times relative to aligned IED centers ===
    % === Ensure aligned_times are available (load from .mat if missing) ===
    if (~isfield(fig.UserData, 'templateAlignedTimes_sec') || isempty(fig.UserData.templateAlignedTimes_sec))
        mat_path = fullfile(fig.UserData.output_dir, ...
            [fig.UserData.subject_id '_' fig.UserData.run_id '_' fig.UserData.ied_id '_alignedTimes.mat']);
        if exist(mat_path, 'file')
            tmp = load(mat_path, 'aligned_times');
            fig.UserData.templateAlignedTimes_sec = tmp.aligned_times;
            fprintf('[DEBUG] Reloaded aligned_times from %s\n', mat_path);
        else
            warning('No aligned_times found in memory or saved file.');
        end
    end

    % === Convert to times relative to aligned IED centers ===
    if isfield(fig.UserData, 'templateAlignedTimes_sec') && ~isempty(fig.UserData.templateAlignedTimes_sec)
        aligned_times = fig.UserData.templateAlignedTimes_sec;  % these are zeros of subplots

        fprintf('[DEBUG] Found %d aligned IED reference times.\n', numel(aligned_times));

        rel_onsets = nan(size(abs_onsets));
        rel_peaks  = nan(size(abs_peaks));

        fprintf('[DEBUG] Matching each onset/peak to nearest aligned time:\n');
        fprintf('-----------------------------------------------------------\n');
        fprintf('Row\tChannel\tAbsOnset\tAbsPeak\tNearestAlign\tRelOnset\tRelPeak\n');

        for i = 1:numel(abs_peaks)
            [~, idx_nearest] = min(abs(aligned_times - abs_peaks(i)));
            rel_onsets(i) = abs_onsets(i) - aligned_times(idx_nearest);
            rel_peaks(i)  = abs_peaks(i)  - aligned_times(idx_nearest);

            fprintf('%d\t%s\t%.3f\t%.3f\t%.3f\t%.3f\t%.3f\n', ...
                i, ch_labels(i), abs_onsets(i), abs_peaks(i), ...
                aligned_times(idx_nearest), rel_onsets(i), rel_peaks(i));
        end
        fprintf('-----------------------------------------------------------\n');
    else
        warning('No templateAlignedTimes found — using raw absolute times.');
        rel_onsets = abs_onsets;
        rel_peaks  = abs_peaks;
    end

    % === Store to GUI data ===
    fig.UserData.loadedOnsetTimes = rel_onsets;
    fig.UserData.loadedPeakTimes  = rel_peaks;
    fig.UserData.loadedChannels   = ch_labels;

    % === Confirmation Message ===
    msg = sprintf(['✅ Loaded onset/peak file:\n%s\n\n',...
                   'Converted to relative times around aligned zero.\n',...
                   'Contains %d channels.\n\nCheck MATLAB command window for detailed debug output.'], ...
                   file, numel(ch_labels));
    uialert(fig, msg, 'Onset/Peak Times Loaded');

    fprintf('[✔] Loaded and aligned onset/peak file: %s (%d channels)\n', ...
            fullpath, numel(ch_labels));

    % === Optional visual debugging ===
    if isfield(fig.UserData, 'axesHandles') && ~isempty(fig.UserData.loadedOnsetTimes)
        try
            plot_loaded_onset_peaks(fig);
            fprintf('[DEBUG] Called plot_loaded_onset_peaks(fig)\n');
        catch plotErr
            fprintf('[DEBUG] plot_loaded_onset_peaks error: %s\n', plotErr.message);
        end
    else
        fprintf('[DEBUG] No axesHandles or empty loaded times — skipping plot overlay.\n');
    end
end



%% all functions in this section are added for the scenrio where the user loads the previously marked ones
function plot_loaded_onset_peaks(fig)
    % Ensure data exists
    if ~isfield(fig.UserData, 'axesHandles') || ~isfield(fig.UserData, 'loadedOnsetPeakTable')
        return;
    end

    tbl = fig.UserData.loadedOnsetPeakTable;
    axesHandles = fig.UserData.axesHandles;
    currentPage = fig.UserData.currentPage;
    tracesPerPage = fig.UserData.tracesPerPage;
    allChNames = fig.UserData.channelnames_bipolar;
    win_sec = fig.UserData.win_sec;

    % Determine which channels are currently visible
    startIdx = (currentPage - 1) * tracesPerPage + 1;
    endIdx = min(startIdx + tracesPerPage - 1, numel(allChNames));
    visibleChNames = allChNames(startIdx:endIdx);

    % Compute center (mean peak time across channels)
    center_time = mean(tbl.AvgPeakTime, 'omitnan');

    % Loop through the visible axes
    for i = 1:length(axesHandles)
        ax = axesHandles(i);
        hold(ax, 'on');

        % Remove old onset/peak markers
        delete(findall(ax, 'Tag', 'onsetLine'));
        delete(findall(ax, 'Tag', 'peakLine'));

        % Skip if no channel for this axis
        if i > numel(visibleChNames)
            continue;
        end
        chName = string(visibleChNames{i});

        % Find corresponding row in Excel table
        rowIdx = find(strcmpi(tbl.Channel, chName));
        if isempty(rowIdx)
            continue; % skip if no match found
        end

        onsetT = tbl.AvgOnsetTime(rowIdx);
        peakT  = tbl.AvgPeakTime(rowIdx);

        % ✅ Convert absolute times to relative (centered at IED)
        onsetRel = onsetT - center_time;
        peakRel  = peakT  - center_time;

        % Clip to visible window range
        onsetRel = max(-win_sec, min(win_sec, onsetRel));
        peakRel  = max(-win_sec, min(win_sec, peakRel));

        yl = ylim(ax);

        % === Onset Line ===
        onsetLine = plot(ax, [onsetRel onsetRel], yl, 'g--', 'LineWidth', 1.5, ...
            'Tag', 'onsetLine', ...
            'UserData', struct('channel', chName, 'type', 'onset'));
        onsetLine.ButtonDownFcn = @(src,evt) start_drag_marker(src, fig);

        % === Peak Line ===
        peakLine = plot(ax, [peakRel peakRel], yl, 'm--', 'LineWidth', 1.5, ...
            'Tag', 'peakLine', ...
            'UserData', struct('channel', chName, 'type', 'peak'));
        peakLine.ButtonDownFcn = @(src,evt) start_drag_marker(src, fig);

        % Labels
        text(peakRel, yl(2)*0.9, 'Peak', 'Parent', ax, ...
            'Color', [0.5 0 0.5], 'FontSize', 8, 'HorizontalAlignment', 'center');
        text(onsetRel, yl(2)*0.8, 'Onset', 'Parent', ax, ...
...
% =============
function drag_motion(ax, lineObj)
    % Track mouse motion and move the line horizontally
    cp = ax.CurrentPoint(1,1);
    lineObj.XData = [cp cp];
end

% =============
function stop_drag_marker(fig, lineObj, ch, type)
    % Stop dragging
    fig.WindowButtonMotionFcn = '';
    fig.WindowButtonUpFcn = '';

    % Record final position
    newX = mean(lineObj.XData);
    tbl = fig.UserData.loadedOnsetPeakTable;
    idx = find(strcmpi(tbl.Channel, ch));
    if isempty(idx)
        return;
    end

    % Update the relevant time in the table
    if strcmp(type, 'onset')
        tbl.AvgOnsetTime(idx) = newX;
    else
        tbl.AvgPeakTime(idx) = newX;
    end

    % Store updated table back into the GUI data
    fig.UserData.loadedOnsetPeakTable = tbl;
    fprintf('[UPDATED] %s %s moved to %.3f s\n', ch, type, newX);

    % === OPTIONAL VISUAL CUE (line thickens briefly when dropped) ===
    try
        lineObj.LineWidth = 3;
        pause(0.15);
    ...
    % === AUTO-SAVE updated onset/peak Excel ===
    try
        % Build save directory (same place where Excel was loaded)
        [ied_folder, ~, ~] = fileparts(fig.UserData.ied_txt_path);
        [parent_dir, ~, ~] = fileparts(ied_folder);
        save_dir = fullfile(parent_dir, 'avg_onset_peak_times');
        if ~exist(save_dir, 'dir')
        ...
        base = sprintf('%s_%s_%s', ...
            ...
            fig.UserData.run_id, ...
            fig.UserData.ied_id);
        out_path = fullfile(save_dir, [base '_avg_onset_peak_times.xlsx']);

        writetable(tbl, out_path);
        fprintf('[💾 Auto-saved] Updated onset/peak table → %s\n', out_path);
    catch ME
        warning('Failed to auto-save onset/peak Excel: %s', ME.message);
    end
end
