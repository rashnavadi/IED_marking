...
function manual_ied_loader_gui()
    ...
    alignAllButton = uibutton(leftLayout, ...
        'Text', 'Align All IEDs (Cross-corr) in the main channel', ...
    ...
    zoomLabel = uilabel(leftLayout, ...
        'Text', 'Time Window:', ...
        'HorizontalAlignment', 'left');

    % Dropdown menu
    zoomDropdown = uidropdown(leftLayout, ...
        'Items', {'±0.5s','±1s','±2s','±5s'}, ...
        'Value', '±0.5s', ...
        'Tag', 'TimeWindowMenu', ...
   ...
   toggleDropdown = uidropdown(leftLayout, ...
    'Items', {'Mark Onset', 'Mark Peak'}, ...
    'ValueChangedFcn', @(dd,event) switch_mark_mode(fig, dd.Value));
    ...
    saveBtn = uibutton(leftLayout, ...
    'Text', 'Save Onset & Peak Times to Excel', ...
    'ButtonPushedFcn', @(btn, event) save_onset_peak(fig), ...
    'FontSize', 11, ...
    'Tooltip', 'Export all marked onset/peak times and amplitudes to Excel');


    fig.UserData.avgTemplate25 = [];      % Initial avg template (channels x time)
    fig.UserData.templateAlignedTimes = [];  % Refined times via cross-corr (IEDs x i main channel)
...
function select_eeg_file(fig)
    ...
    % === Initialize peak and onset times ===
    nChannels = fig.UserData.totalChannels;
    fig.UserData.peakTimes  = nan(1, nChannels);  % one peak time per channel
    fig.UserData.onsetTimes = nan(1, nChannels);  % one onset time per channel
...
function select_manual_ied_file(fig)
    load_adjusted_ieds_file(fig);  % First load adjusted IEDs
    ...
    % Reset templates to avoid contamination across subjects
    fig.UserData.avgTemplate25 = [];
    fig.UserData.finalAvgTemplate = [];

    % Only reset peak/onset arrays if EEG data is about to be reloaded
    if isfield(fig.UserData, 'totalChannels') && fig.UserData.totalChannels > 0
        nChannels = fig.UserData.totalChannels;
        fig.UserData.onsetTimes = nan(1, nChannels);
        fig.UserData.peakTimes = nan(1, nChannels);
    else
        % If channel count not known yet, don't reset — keep user's markings
        warning('[⚠️] EEG channel count not set. Skipping reset of onset/peak times.');
    end
...
%% function 6 
% 1- Plots the template passed in via template_key (either:
    % 'avgTemplate25': the average of manually marked IEDs (single channel only), or
    % 'finalAvgTemplate': the full [nChannels x time] result from compute_final_avg).
% 2- It:
    % Handles paging, showing a subset of channels.
    % Colors the main channel green.
    % Adds onset and peak lines if available.
    % Adjusts y-limits.
    % Uses fixed default x-axis: ±0.5s (even if the user changes it in the dropdown).
...
function plot_avg_template(fig, template_key)
    ...
    win_sec = get_time_window(fig);  % dynamically get time window
    t = linspace(-win_sec, win_sec, nTime);  % use it for plotting
    ...
    for i = 1:length(axesHandles)
        ...
        if i <= length(visibleIdx) && visibleIdx(i) <= size(avg_data, 1)
            ...
            % === Always show peak and onset lines, even outside current zoom ===
            full_t = linspace(-fig.UserData.win_sec, fig.UserData.win_sec, size(avg_data, 2));  % full window time

            % Plot peak
            peak_time = fig.UserData.peakTimes(chIdx);
            if ~isnan(peak_time)
                xline(ax, peak_time, '--', 'Color', [0.5 0 0.8], 'LineWidth', 2);  % purple
            end

            % Plot onset
            onset_time = fig.UserData.onsetTimes(chIdx);
            if ~isnan(onset_time)
                xline(ax, onset_time, '--', 'Color', [0 0.6 0], 'LineWidth', 2);  % green
            end
            ...
            % Plot peak
            if chIdx <= length(fig.UserData.peakTimes) && ~isnan(fig.UserData.peakTimes(chIdx))
                xline(ax, fig.UserData.peakTimes(chIdx), '--', 'Color', [0.5 0 0.8], 'LineWidth', 2);  % purple
            end

            % Plot onset
            if chIdx <= length(fig.UserData.onsetTimes) && ~isnan(fig.UserData.onsetTimes(chIdx))
                xline(ax, fig.UserData.onsetTimes(chIdx), '--', 'Color', [0 0.6 0], 'LineWidth', 2);  % green
            end
...
% Aim: To make all IEDs consistently aligned in time (in the marking channle) before averaging
% Aligns all originally marked IEDs to a consistent shape using
% cross-correlation with avgTemplate25 in the main channel and find the new
% ied timings and use that timing for all the channels
...
function align_all_ieds(fig)
    ...
    try
        % Align all original IEDs using cross-correlation with
        % avgTemplate25 only in the main channel that was used for the
        % manula marking
        fs = fig.UserData.fs;
        eeg = fig.UserData.eeg_data;
        ...
        % Get main channel used for manual marking
        main_label = strtrim(fig.UserData.marking_channel_used);
        main_idx = find(strcmp(channelnames, main_label));
    
        % Extract channel name parts
        parts = split(main_label, '-');
        ch1 = parts{1};
        ch2 = parts{2};

        % Ensure both monopolar components exist
        if ~isKey(mono_map, ch1) || ~isKey(mono_map, ch2)
            uialert(fig, 'Main channel not found in monopolar channel list.', 'Channel Error');
            close(wait);
            return;
        end
        ...
        % Timing parameters
        win_sec = get_time_window(fig);  % e.g., ±0.5s
        pre_samples = round(win_sec * fs);
        post_samples = round(win_sec * fs);

        n_ieds = numel(adjusted_times);
        % 1. Template: -200 to +200 ms
        template_len = round(0.4 * fs);   % 400 ms window
        half_template = round(template_len / 2);
        
        % 2. Lag search range: ±100 ms (i.e., 250 samples)
        search_margin = round(0.1 * fs);  % ±100ms around each IED center to avoid reaching to the neighbouring IED
       
        aligned_times = nan(n_ieds, 1);  % Only for main channel

        % === Main alignment loop for the main channel only ===
        for i = 1:n_ieds
            if isnan(manual_times(i))
                % Align adjusted IED using cross-correlation
                center = round(adjusted_times(i) * fs);
            else
                % Already manually marked — use directly
                aligned_times(i) = round(manual_times(i) * fs);
                continue;
            end

            % Extract segment around adjusted IED time
            % Get EEG segment: must be long enough to support all lags
            % Length needed: template_len + 2 * search_margin
            segment_len = template_len + 2 * search_margin;
            start_idx = center - round(segment_len/2);
            end_idx   = center + round(segment_len/2) - 1;

            if start_idx < 1 || end_idx > size(eeg, 2)
                continue;  % Skip if bounds are invalid
            end

            % Extract EEG segment around adjusted time
            segment = eeg(idx1, start_idx:end_idx) - eeg(idx2, start_idx:end_idx);
            % Ensure equal length before xcov
            if length(segment) ~= length(avg_template)
                min_len = min(length(segment), length(avg_template));
                segment = segment(1:min_len);
                template = avg_template(1:min_len);
            else
                template = avg_template;
            end

            % Perform cross-correlation: slide template ±250 samples (±100 ms)
            [cross_corr, lags] = xcov(segment, template, search_margin, 'coeff'); % This slides the template25 over ±100 ms relative to the center.
            [~, max_idx] = max(cross_corr);
            best_lag = lags(max_idx);  % in samples

            % Update aligned time
            aligned_times(i) = center + best_lag;

            fprintf('[DEBUG] IED %d | center = %d | lag = %+d | aligned = %d\n', ...
                i, center, best_lag, aligned_times(i));
            fprintf('[DEBUG] IED %d | center = %d | lag = %+d samples (%.2f ms) | aligned = %d\n', ...
                i, center, best_lag, 1000*best_lag/fs, aligned_times(i));
        end

        % === Store result ===
        aligned_times = round(aligned_times);
        fig.UserData.templateAlignedTimes = aligned_times;

        fprintf('[✔] Stored %d aligned IEDs in templateAlignedTimes (main channel only).\n', numel(aligned_times));
        fprintf('[ℹ️] Includes %d manually marked and %d aligned IEDs.\n', ...
            sum(~isnan(manual_times)), sum(isnan(manual_times)));

        % === Save aligned times (seconds) ===
        aligned_secs = aligned_times / fs;
        save_path = fullfile(fig.UserData.output_dir, ...
            sprintf('%s_%s_%s_final_aligned.txt', ...
            fig.UserData.subject_id, fig.UserData.run_id, fig.UserData.ied_id));
...
% 1- Loads templateAlignedTimes (aligned IED sample indices) — these are typically aligned using the main channel.
% 2- For each channel in the bipolar EEG:
    % a- It extracts EEG segments centered at each aligned IED time.
    % b- Computes the average waveform per channel over all aligned IEDs.
% 3- Stores the result in fig.UserData.finalAvgTemplate → a [nChannels × time] matrix.
% 4- Also:
    % Automatically detects peak times per channel (around ±0.1s).
    % Saves those peak times to disk.
% 5- Calls plot_avg_template(fig, 'finalAvgTemplate') to plot the result.
% Aim: This function does IED averaging across all channels based on updated IED timings from the main channel.

function compute_final_avg(fig)
    % This assumes that fig.UserData.templateAlignedTimes is already computed
    % and contains a matrix: [nIEDs x 1 channel/main channel] of refined sample indices
    % This assumes fig.UserData.templateAlignedTimes contains [nIEDs x 1] aligned sample indices for the main channel

    eeg = fig.UserData.eeg_data;
    fs = fig.UserData.fs;
    aligned_samples = fig.UserData.templateAlignedTimes; % [nIEDs x 1]
    channelnames = fig.UserData.channelnames_bipolar;
    mono_map = containers.Map(fig.UserData.channelnames_mono, 1:numel(fig.UserData.channelnames_mono));
    ...
    post_samples = round(win_sec * fs);
    win_len = pre_samples + post_samples + 1;
    n_channels = numel(channelnames);    % number of bipolar channels
    n_ieds = size(aligned_samples, 1);  % number of IEDs
    ...
    uialert(fig, 'Final average computed. You can now manually mark peaks and onsets.', ...
        'Ready for Manual Marking');

    % Reset peak/onset times for fresh marking
    fig.UserData.peakTimes = nan(1, size(final_avg, 1));
    fig.UserData.onsetTimes = nan(1, size(final_avg, 1));
...
function handle_click_on_avg(fig, src, ~)
    ...
    % ✅ Must get cursor location and axis
    click_ax = src;
    if ~isa(click_ax, 'matlab.ui.control.UIAxes')
        disp('[⚠️] Click target is not a UIAxes object.');
        return;
    end

    % ✅ Get X coordinate (time in seconds)
    cp = click_ax.CurrentPoint;
    x_click = cp(1, 1);  % Clicked time (in seconds)

    % ✅ Identify which channel (based on which axis was clicked)
    ...
    if isempty(ax_idx)
        disp('[⚠️] Could not find clicked axis in the list.');
        return;
    end
    ch_idx = ch_offset + ax_idx;

    % ✅ Determine marking mode
    if fig.UserData.isMarkingOnset
        label_type = 'onset';
        target_array = fig.UserData.onsetTimes;
        clr = [0 0.6 0];  % green
    elseif fig.UserData.isMarkingPeak
        label_type = 'peak';
        target_array = fig.UserData.peakTimes;
        clr = [0.5 0 0.8];  % magenta
    else
        disp('[⚠️] Not in marking mode.');
        return;
    end

    % ✅ Ask to overwrite if already marked
    alreadyMarked = ~isnan(target_array(ch_idx));
    if alreadyMarked
        choice = uiconfirm(fig, ...
            sprintf('%s already marked on channel %d.\nReplace?', upper(label_type), ch_idx), ...
            ['Replace ' upper(label_type)], ...
            'Options', {'Yes', 'No'}, ...
            'DefaultOption', 2);
        if strcmp(choice, 'No')
            return;
    ...
    % ✅ Save the marking
    if strcmp(label_type, 'onset')
        fig.UserData.onsetTimes(ch_idx) = x_click;
    else
        fig.UserData.peakTimes(ch_idx) = x_click;
    end

    % ✅ Redraw all plots for this page
    template_key = 'finalAvgTemplate';  % or 'avgTemplate25'
    plot_avg_template(fig, template_key);
...
function save_onset_peak(fig)
    ...
    t = linspace(-0.5, 0.5, size(avg_data, 2));  % assumed time vector

    for i = 1:numel(ch_names)
        if ~isnan(peak(i)) && ~isempty(avg_data)
            [~, idx] = min(abs(t - peak(i)));
            peak_amp(i) = avg_data(i, idx);
        end
    end

    T = table(ch_names(:), peak_amp(:), onset(:), ...
        'VariableNames', {'Channel', 'PeakAmplitude', 'OnsetTime'});

    save_dir = fullfile(fileparts(fileparts(fig.UserData.ied_txt_path)), 'avg_onset_peak_times');
    if ~exist(save_dir, 'dir'), mkdir(save_dir); end

    filename = sprintf('%s_%s_%s_avg_onset_peak_times.xlsx', subj, run, ied);
    fullpath = fullfile(save_dir, filename);
    writetable(T, fullpath);

    uialert(fig, sprintf('Saved to:\n%s', fullpath), 'Excel Saved');
end
...
%% function 17
% his function visualizes the per-channel average IED waveforms, 
% sorted by either onset or peak latency, and saves the result. 
% It's mainly used for quality control or interpretation, to see how waveform shapes vary with timing across channels.
...
function changeTimeWindow(fig)
    disp('[UI] Time window changed from dropdown.');

    % Parse new time window value (e.g., '±0.5s' → 0.5)
    dd = findobj(fig, 'Tag', 'TimeWindowMenu');
    val_str = dd.Value;  % e.g., '±0.5s'
    tokens = regexp(val_str, '±([\d.]+)s', 'tokens');
    if ~isempty(tokens)
        fig.UserData.win_sec = str2double(tokens{1}{1});
        fprintf('[UI] Updated time window: ±%.2f seconds\n', fig.UserData.win_sec);
    else
        warning('⚠️ Could not parse time window from dropdown value: %s', val_str);
        fig.UserData.win_sec = 0.5;  % fallback
    end

    % Only refresh plot if final average exists
    if isfield(fig.UserData, 'finalAvgTemplate') && ~isempty(fig.UserData.finalAvgTemplate)
        compute_final_avg(fig); 
    else
        disp('[ℹ️] finalAvgTemplate not found — skipping plot refresh.');
    end
end

%% function 20 
function win_sec = get_time_window(fig)
    % Try to get the dropdown by its Tag
    dropdown = findobj(fig, 'Tag', 'TimeWindowMenu');

    if isempty(dropdown) || ~isprop(dropdown, 'Value')
        warning('⚠️ Time window dropdown not found — using default 0.5s.');
        win_sec = 0.5;
        return;
    end

    % Get dropdown value, like '±0.5s' or '±1s'
    raw_val = dropdown.Value;

    % Parse number from string (e.g., '±1s' → 1.0)
    win_sec = sscanf(raw_val, '±%fs');

    % Fallback if parsing fails
    if isempty(win_sec) || isnan(win_sec)
        win_sec = 0.5;
    end
end

%% function 21
function switch_mark_mode(fig, selectedMode)
    if strcmp(selectedMode, 'Mark Onset')
        fig.UserData.isMarkingOnset = true;
        fig.UserData.isMarkingPeak = false;
        fig.Pointer = 'crosshair';
        uialert(fig, 'Click on a waveform to mark the ONSET', 'Mark Onset');   
        disp('[🟢] Marking mode set to: ONSET');

    elseif strcmp(selectedMode, 'Mark Peak')
        fig.UserData.isMarkingOnset = false;
        fig.UserData.isMarkingPeak = true;
        fig.Pointer = 'crosshair';
        uialert(fig, 'Click on a waveform to mark the PEAK', 'Mark Peak'); 
        disp('[🟣] Marking mode set to: PEAK');

    else
        fig.UserData.isMarkingOnset = false;
        fig.UserData.isMarkingPeak = false;
        fig.Pointer = 'arrow';
        warning('[⚠️] Unknown marking mode selected');
    end

    % Refresh plot to show all vertical lines (onset + peak)
    template_key = 'finalAvgTemplate';
    plot_avg_template(fig, template_key);
end
